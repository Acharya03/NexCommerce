...
const loginUser = asyncHandler(async(req, res) => {
    ...
    const existingUser = await User.findOne({email}); //mongoose searching for user email in database

    if(existingUser) {
        const isPasswordValid = await bcrypt.compare(password, existingUser.password); //password verification
